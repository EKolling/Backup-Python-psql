import os
from FLU import backup_env_files, create_local_backup, verify_local_backup_content, listar_arquivos_antigos, remover_arquivos
...
diretorio_de_projeto = os.getenv('PROJECTS_DIR')
...
################ inicio do backup
    ...
    arquivo_backup = create_local_backup(infodb)
    ...
    print("---------------- backup .env Iniciada ----------")

    # Criar uma instância de InfoTempo
    infotempo = InfoTempo()

    # Chamada da função para realizar o backup .env
    backup_env_files(infotempo, infodb, diretorio_de_projeto, diretorio_de_backup)

    print("---------------- envio de informações para o Zabbix ----------")
    ...
    try:
        ...
        zbx.send(metrics)
        print("Dados enviados com sucesso para o Zabbix.")

    except Exception as e:
        print(f"Erro ao enviar dados para o Zabbix: {str(e)}")
